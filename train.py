...
from models.model_qiskit_pqc import QiskitPQCClassifier
from models.model_qkernel import QuantumKernelClassifier
from models.model_qnn import QuantumNeuralNetwork
from models.model_qnn_advanced import AdvancedQuantumNeuralNetwork
from models.model_qpf import UNet3DWithQPF
...
from models.model_vqc import VariationalQuantumClassifier
...
def train(
    ...
    elif config.MODE == "Resnet3D":
        model = ResNet3D(config.RESNET_DEPTH).to(device)
    elif config.MODE == "UNET2D":
        model = UNet2D().to(device)
    elif config.MODE == "UNET3D":
        model = UNet3D(in_channels=1, num_classes=1).to(device)
    elif config.MODE == "3DCNN" :
        model = Efficient3DCNN().to(device)
    elif config.MODE == "UNet3DWithQPF":
        model = UNet3DWithQPF(in_channels=1, num_classes=1).to(device)
    elif config.MODE == "QKernel":
        model = QuantumKernelClassifier(n_qubits=config.N_QUBITS).to(device)
    elif config.MODE == "QNN":
        model = QuantumNeuralNetwork(n_qubits=4).to(device)
    elif config.MODE == "Inception":
        model = InceptionV3(pretrained=True).to(device)
    elif config.MODE == "VQC":
        model = VariationalQuantumClassifier(
            n_qubits=config.N_QUBITS, 
            n_layers=config.N_LAYERS
        ).to(device)
    elif config.MODE == "AdvancedQNN":
        model = AdvancedQuantumNeuralNetwork(
            n_qubits=config.QNN_N_QUBITS, 
            n_layers=config.QNN_N_LAYERS
        ).to(device)
    elif config.MODE == "QiskitPQC":
        # SPECIAL STEP: Pre-fit PCA and Scaler before creating the model
        logging.info("Fitting PCA and Scaler for QiskitPQC model...")
        
        # Use a temporary, non-shuffled loader to get all training data
        temp_loader = get_data_loader(config.DATADIR, train_df, mode=config.MODE)
        
        all_images = []
        for batch in tqdm(temp_loader, desc="Extracting data for fitting transformers"):
            all_images.append(batch['image'].cpu().numpy())
        
        X_all_flat = np.vstack(all_images).reshape(len(train_df), -1)
        
        # Fit the transformers
        pca = PCA(n_components=config.PQC_N_FEATURES)
        scaler = StandardScaler()
        X_pca = pca.fit_transform(X_all_flat)
        scaler.fit(X_pca) # Fit the scaler on the PCA-transformed data
        
        logging.info("Transformers fitted. Instantiating QiskitPQCClassifier.")
        
        # Now, instantiate the model with the FITTED transformers
        model = QiskitPQCClassifier(
            n_features=config.PQC_N_FEATURES, 
